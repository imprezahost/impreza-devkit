...
- **`openapi/asyncapi.yaml`** — AsyncAPI 3.0 spec documenting the
  webhook delivery contract. Covers the 16 event types
  ...
  standard delivery headers. Cross-linked from `openapi.yaml`, both
  package READMEs, and the root README.
- **Note on event firing status:** as of this version, only
  `webhook.test` is actively fired (delivery probe on subscription
  create). The other 15 event types are documented as the public
  contract; the server-side hooks that fire them are being wired
  progressively. The wire shape is stable — subscribers can register
  for any event today; they'll start receiving them as the hooks
  land server-side.
- **`cli-go/` — Go single-binary CLI (`impreza`)**, source-tree
  ready and Phase 7.6 release-prep done (goreleaser config + GitHub
  Actions workflow). Releases ship on the independent
  `cli-go-v<version>` tag track (separate from the Python lock-step
  `sdk-v` / `cli-v` cycle) and are uploaded to GitHub Releases as
  five static-binary archives (Linux x86_64+arm64, macOS
  x86_64+arm64, Windows x86_64), each bundling the binary plus
  `README.md`, `LICENSE`, `openapi.yaml`, and `asyncapi.yaml`. The
  Python CLI remains the reference implementation; both CLIs share
  the same TOML config and verb surface (80 verbs across 10 resource
  groups), so users can install both side by side. Pre-release
  binaries pending the first `cli-go-v0.1.0` tag push.

No code changes to the Python packages — docs + Go-CLI bootstrap
only. No PyPI bump until the next feature release picks this up.
