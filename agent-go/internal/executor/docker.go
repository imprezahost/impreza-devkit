...
type Docker struct {
	...
	SaveReplacement      func(*sdkclient.PollCommand, *ReplacementWork) error
...
func (d *Docker) deploy(ctx context.Context, cmd *sdkclient.PollCommand) (result sdkclient.DeployResult) {
	...
	if d.SupervisePreparation && d.SaveReplacement != nil && recovery != nil && recovery.Phase == "replacing" && p.Manifest.Runtime.RestoreDatabase == nil {
		...
		if err = d.SaveReplacement(cmd, work); err != nil {
			return failResult(cmd.ID, "persist supervised replacement: "+err.Error())
		}
		// Once the durable identity exists, a lost launch response is ambiguous.
		// Never restore preparation or fall back to synchronous execution here.
		runtimeStarted = true
		if err = d.launchReplacementWork(ctx, work); err != nil {
			d.Log.Warn("replacement launch awaiting reconciliation", "err", err)
