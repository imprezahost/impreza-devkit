...
func TestPollReportsUnsupportedAndContinues(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/v1/agent/poll":
			...
			if err := json.NewDecoder(r.Body).Decode(&request); err != nil || len(request.Capabilities) != 10 || request.Capabilities[0] != "startup-health-v1" || request.Capabilities[1] != "deploy-cancel-v1" || request.Capabilities[2] != "build-secrets-v1" || request.Capabilities[3] != "compose-source-files-v1" || request.Capabilities[4] != sdkclient.ServiceBindingProtocol || request.Capabilities[5] != sdkclient.ServiceBindingRetirementProtocol || request.Capabilities[6] != sdkclient.ServiceBindingGenerationProtocol || request.Capabilities[7] != sdkclient.ServiceBindingGenerationRetirementProtocol || request.Capabilities[8] != sdkclient.ServiceBindingRotationProtocol || request.Capabilities[9] != sdkclient.DeploymentProgressProtocol {
