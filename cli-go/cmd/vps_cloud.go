...
// 7.4 lands images / rescue / iso / rdns / ssh-keys sub-groups +
// 5 inline verbs (vnc / vnc-password / resize / boot-order / ipv6).

var vpsCloudCmd = &cobra.Command{
	Use:   "cloud",
	Short: "Cloud-only sub-resources (images, rescue, iso, rdns, ssh-keys + inline verbs).",
}
...
var vpsCloudRdnsCmd = &cobra.Command{
	Use:   "rdns",
	Short: "Reverse-DNS (PTR record) management for Cloud VPS IPs.",
}

var vpsCloudRdnsGetCmd = &cobra.Command{
	Use:   "get <ip>",
	Short: "Read the rDNS hostname for an IP. (May be WAF-blocked; see plan §3.5 follow-up.)",
	Args:  cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		c, _, err := newClient()
		if err != nil {
		...
		data, err := c.CloudRdnsGet(cmd.Context(), args[0])
		if err != nil {
			return err
		}
		f, err := resolveFormat()
		...
		if f != output.FormatTable {
			return renderJSONOrYAML(cmd.OutOrStdout(), data, f)
		}
		t := output.NewTable(cmd.OutOrStdout())
...
var vpsCloudRdnsSetCmd = &cobra.Command{
	Use:   "set <ip> <hostname>",
	Short: "Set the rDNS hostname for an IP.",
	Args:  cobra.ExactArgs(2),
	RunE: func(cmd *cobra.Command, args []string) error {
		c, _, err := newClient()
		if err != nil {
		...
		if _, err := c.CloudRdnsSet(cmd.Context(), args[0], args[1]); err != nil {
			return err
		}
		output.Success("rDNS for %s set to %s.", args[0], args[1])
		return nil
	},
}

var vpsCloudRdnsDeleteCmd = &cobra.Command{
	Use:     "delete <ip>",
	Aliases: []string{"rm", "remove"},
	Short:   "Remove the rDNS entry for an IP.",
	Args:    cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		c, _, err := newClient()
		if err != nil {
		...
		if err := c.CloudRdnsDelete(cmd.Context(), args[0]); err != nil {
			return err
		}
		output.Success("rDNS entry for %s removed.", args[0])
		return nil
	},
}
...
func init() {
	...
	// rdns
	vpsCloudRdnsCmd.AddCommand(vpsCloudRdnsGetCmd, vpsCloudRdnsSetCmd, vpsCloudRdnsDeleteCmd)
