...
func TestAuthHeadersAreInjected(t *testing.T) {
	...
	const prefix = "impreza-sdk-go/"
	if gotUA == "" || len(gotUA) <= len(prefix) || gotUA[:len(prefix)] != prefix {
		t.Errorf("User-Agent = %q, want prefix %q", gotUA, prefix)
	}
}

func TestSetUserAgentOverridesPrefix(t *testing.T) {
	// Reset to default after the test so other tests in this package
	// see the expected "impreza-sdk-go" prefix.
	t.Cleanup(func() { SetUserAgent("impreza-sdk-go") })

	SetUserAgent("impreza-cli-go")
